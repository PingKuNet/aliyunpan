...
keyboardStore.$subscribe((_m: any, state: KeyboardState) => {
  ...
  if (TestKeyboardSelect(state.KeyDownEvent, viewlist.value, uploadedStore, null)) return
})

const handleSelectAll = () => uploadedStore.mSelectAll()
const handleOrder = (order: string) => uploadedStore.mOrderListData(order)
const handleSelect = (shareid: string, event: any) => {
  onHideRightMenuScroll()
  uploadedStore.mMouseSelect(shareid, event.ctrlKey, event.shiftKey)
}

const handleDelete = () => uploadedStore.mDeleteUploaded([...uploadedStore.ListSelected])

const handleOpenFile = (file: IStateUploadFile | null) =>
  uploadedStore.mOpenUploadedFile(file, [...uploadedStore.ListSelected], false)

const handleOpenDir = (file: IStateUploadFile | null) =>
  uploadedStore.mOpenUploadedFile(file, [...uploadedStore.ListSelected], true)

const handleDeleteAll = () => uploadedStore.mDeleteAllUploaded()
...
const handleRightClick = (e: { event: MouseEvent; node: any }) => {
  let key = e.node.key
  if (!uploadedStore.ListSelected.has(key)) uploadedStore.mMouseSelect(key, false, false)
  onShowRightMenu('uploadedrightmenu', e.event.clientX, e.event.clientY)
}
</script>
...
<template>
  ...
  <div class="toppanbtns" style="height: 26px">
    ...
    <div class="flex flexnoauto cellcount" title="总共已上传完记录数">
      <a-badge color="#637dff" :text="'总记录数 ' + uploadedStore.ListStats.count" />
    </div>
  </div>
  <div style="height: 14px"></div>
  <div class="toppanbtns" style="height: 26px">
    <div class="toppanbtn" v-show="uploadedStore.IsListSelected">
      ...
      <a-button type="text" size="small" tabindex="-1" @click="handleDeleteAll"><i class="iconfont icondelete" />清除全部上传记录</a-button>
    </div>
    <div class="toppanbtn" v-show="!uploadedStore.IsListSelected">
      <a-button type="text" size="small" tabindex="-1" @click="handleDeleteAll"><i class="iconfont icondelete" />清除全部上传记录</a-button>
    </div>
    ...
    <div style="margin: 0 3px">
      <AntdTooltip title="点击全选" placement="left">
        <a-button shape="circle" type="text" tabindex="-1" class="select all" @click="handleSelectAll" title="Ctrl+A">
          <i :class="uploadedStore.IsListSelectedAll ? 'iconfont iconrsuccess' : 'iconfont iconrpic'" />
        </a-button>
      </AntdTooltip>
    </div>
    <div class="selectInfo" style="min-width: 266px">{{ uploadedStore.ListDataSelectCountInfo }}</div>
  </div>
  <div class="toppanlist" @keydown.space.prevent="() => true">
          ...
          >
            <div style="margin: 2px">
              <a-button shape="circle" type="text" tabindex="-1" class="select" :title="index" @click.prevent.stop="handleSelect(item.UploadID, { ctrlKey: true, shiftKey: false })">
                <i :class="uploadedStore.ListSelected.has(item.UploadID) ? 'iconfont iconrsuccess' : 'iconfont iconrpic'" />
              </a-button>
            </div>
            <div class="fileicon">
            ...
            <div class='toppanbtn'>
              <a title='打开文件' @click="handleOpenFile(item)"><i class="iconfont iconwenjian" /></a>&nbsp;&nbsp;
              <a title='打开目录' @click="handleOpenDir(item)"><i class="iconfont iconfolder" /></a>&nbsp;&nbsp;
    ...
    <a-dropdown id="uploadedrightmenu" class="rightmenu" :popup-visible="true" tabindex="-1" :draggable="false" style="z-index: -1; left: -200px; opacity: 0">
      <template #content>
