...
const handleSelect = (shareid: string, event: any) => {
  ...
  uploadingStore.mMouseSelect(shareid, event.ctrlKey, event.shiftKey)
}

const handleStart = () => uploadingStore.mStartUploading()

const handleStartAll = () => uploadingStore.mStartAllUploading()

const handleStop = () => uploadingStore.mStopUploading()

const handleStopAll = () => uploadingStore.mStopAllUploading()

const handleDelete = () => uploadingStore.mDeleteUploading([...uploadingStore.ListSelected])

const handleDeleteAll = () => uploadingStore.mDeleteAllUploading()

const handleTop = () => uploadingStore.mOrderUploading([...uploadingStore.ListSelected])

const handleSearchInput = (value: string) => {
  uploadingStore.mSearchListData(value)
...
const handleRightClick = (e: { event: MouseEvent; node: any }) => {
  ...
  onShowRightMenu('uploadingrightmenu', e.event.clientX, e.event.clientY)
}
</script>
...
<template>
  ...
  <div class="toppanbtns" style="height: 26px">
    ...
    <div class="flex flexnoauto cellcount" title="总共文件数量">
      <a-badge color="#637dff" :text="'总数 ' + uploadingStore.ListStats.count" />
    </div>
    <div class="flex flexnoauto cellcount" title="正在执行上传数">
      <a-badge color="#637dff" :text="'上传 ' + uploadingStore.ListStats.runningCount" />
    </div>
    <div class="flex flexnoauto cellcount" title="总共文件大小">
      <a-badge color="#637dff" :text="'大小 ' + uploadingStore.ListStats.totalSizeStr" />
    </div>
  </div>
  <div style="height: 14px"></div>
  <div class="toppanbtns" style="height: 26px">
    <div class="toppanbtn" v-show="uploadingStore.IsListSelected">
    ...
    <div class="toppanbtn">
      <a-input-search
        ...
        placeholder="快速筛选"
        :model-value="uploadingStore.ListSearchKey"
    ...
    <div style="margin: 0 3px">
      <AntdTooltip title="点击全选" placement="left">
        <a-button shape="circle" type="text" tabindex="-1" class="select all" @click="handleSelectAll" title="Ctrl+A">
          <i :class="uploadingStore.IsListSelectedAll ? 'iconfont iconrsuccess' : 'iconfont iconrpic'" />
        </a-button>
      </AntdTooltip>
    </div>
    <div class="selectInfo" style="min-width: 266px">{{ uploadingStore.ListDataSelectCountInfo }}</div>
  </div>
  <div class="toppanlist" @keydown.space.prevent="() => true">
    <a-list
      ...
      :loading="uploadingStore.ListLoading"
      tabindex="-1"
    ...
    >
      <template #item="{ item, index }">
        <div :key="item.UploadID" class="listitemdiv" :data-id="item.UploadID">
          <div
            :class="'fileitem' + (uploadingStore.ListSelected.has(item.UploadID) ? ' selected' : '') + (uploadingStore.ListFocusKey == item.UploadID ? ' focus' : '')"
          ...
          >
            <div style="margin: 2px">
              <a-button shape="circle" type="text" tabindex="-1" class="select" :title="index" @click.prevent.stop="handleSelect(item.UploadID, { ctrlKey: true, shiftKey: false })">
                <i :class="uploadingStore.ListSelected.has(item.UploadID) ? 'iconfont iconrsuccess' : 'iconfont iconrpic'" />
              </a-button>
            </div>
            <div class="fileicon">
    ...
    <a-dropdown id="uploadingrightmenu" class="rightmenu" :popup-visible="true" tabindex="-1" :draggable="false" style="z-index: -1; left: -200px; opacity: 0">
      <template #content>
