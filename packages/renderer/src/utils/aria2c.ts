...
export async function AriaGetDowningList() {
  ...
  try {
    ...
    DebugLog.mSaveLog('danger', 'AriaGetDowningList' + (e.message || ''))
    SetAriaOnline(false)
  }
}
...
export async function AriaAddUrl(file: IStateDownFile): Promise<string> {
  try {
    ...
    if (info.isDir) {
      ...
      if (!info.ariaRemote) {
        ...
        await fsPromises.mkdir(dirfull, { recursive: true }).catch((e: any) => {
          ...
          DebugLog.mSaveLog('danger', 'AriaAddUrl创建文件夹失败：' + dirfull + ' ' + (e || ''))
          return undefined
        })
      }
      ...
      if (!downurl) {
        ...
        if (typeof durl == 'string') return '生成下载链接失败,' + durl
          ...
          DebugLog.mSaveLog('danger', info.file_id + '生成下载链接失败,' + JSON.stringify(durl))
    ...
    SetAriaOnline(false)
    DebugLog.mSaveLog('danger', 'AriaAddUrl' + (e.message || ''))
  ...
  if (data.inputfile == data.movetofile) {
    ...
    try {
      ...
      try {
        ...
        DebugLog.mSaveLog('danger', 'AriaRename file=' + data.inputfile + ' error=' + (e.message || ''))
      }
    }
  }
